...
export default function weldingsPagerReducer(state = initialState, action) {
  var pagerObj;
  switch (action.type) {

    case WELDINGS_PAGE_CHANGE:
    ...
    case WELDINGS_PER_PAGE_CHANGE:
      if (action.payload.itemsPerPage === state.itemsPerPage) {
        return state;
      }
      pagerObj = getPagerObj(action.payload.itemsPerPage, action.payload.itemCount, 0);
      return pagerObj;
